import {
  Meta,
  Story,
  Canvas,
  ArgsTable,
  Description,
} from "@storybook/addon-docs";
import { html } from "lit-html";
import readme from "./readme.md";

<Meta title="Web Components/Popover menu" component="Popover Menu" />

<Description markdown={readme} />

### Default

<Canvas>
  <Story name="Default" parameters={{ loki: { skip: true } }}>
    {(args) =>
      html`
        <ic-button id="button-1" onclick="buttonClick()"
          >Show popover</ic-button
        >
        <script>
          var icPopover = document.querySelector("ic-popover-menu");
          function buttonClick() {
            icPopover.open = true;
          }
        </script>
        <div>
          <ic-popover-menu anchor="button-1" menu-aria-label="popover">
            <ic-menu-button label="Copy code" disabled="true"></ic-menu-button>
            <ic-menu-group label="View">
              <ic-menu-button
                label="Zoom in"
                keyboard-shortcut="Cmd+"
              ></ic-menu-button>
              <ic-menu-button
                label="Zoom out"
                keyboard-shortcut="Cmd-"
              ></ic-menu-button>
            </ic-menu-group>
            <ic-menu-button
              label="Actions"
              submenu-trigger-for="abc"
            ></ic-menu-button>
          </ic-popover-menu>
          <ic-popover-menu submenu-id="abc">
            <ic-menu-button label="Edit"></ic-menu-button>
            <ic-menu-button
              label="Find"
              submenu-trigger-for="abc123"
            ></ic-menu-button>
            <ic-menu-button
              label="Delete"
              variant="destructive"
            ></ic-menu-button>
          </ic-popover-menu>
          <ic-popover-menu submenu-id="abc123">
            <ic-menu-button
              label="Search the web"
              description="This will search the web to find the thing you are looking for."
            ></ic-menu-button>
            <ic-menu-button label="Find..."></ic-menu-button>
            <ic-menu-button label="Find icons">
              <svg
                slot="icon"
                xmlns="http://www.w3.org/2000/svg"
                height="24px"
                viewBox="0 0 24 24"
                width="24px"
              >
                <path d="M0 0h24v24H0V0z" fill="none" />
                <path
                  d="M17.65 6.35C16.2 4.9 14.21 4 12 4c-4.42 0-7.99 3.58-7.99 8s3.57 8 7.99 8c3.73 0 6.84-2.55 7.73-6h-2.08c-.82 2.33-3.04 4-5.65 4-3.31 0-6-2.69-6-6s2.69-6 6-6c1.66 0 3.14.69 4.22 1.78L13 11h7V4l-2.35 2.35z"
                />
              </svg>
            </ic-menu-button>
            <ic-menu-button
              label="Show found results"
              variant="toggle"
            ></ic-menu-button>
          </ic-popover-menu>
        </div>
      `
    }
  </Story>
</Canvas>

### Max height

<Canvas>
  <Story name="Max height" parameters={{ loki: { skip: true } }}>
    {(args) =>
      html`
        <style>
          ic-popover-menu {
            --max-height: 100px;
          }
        </style>
        <ic-button id="button-1" onclick="buttonClick()"
          >Show popover</ic-button
        >
        <script>
          var icPopover = document.querySelector("ic-popover-menu");
          function buttonClick() {
            icPopover.open = true;
          }
        </script>
        <div>
          <ic-popover-menu anchor="button-1" menu-aria-label="popover">
            <ic-menu-button label="Copy code" disabled="true"></ic-menu-button>
            <ic-menu-group label="View">
              <ic-menu-button
                label="Zoom in"
                keyboard-shortcut="Cmd+"
              ></ic-menu-button>
              <ic-menu-button
                label="Zoom out"
                keyboard-shortcut="Cmd-"
              ></ic-menu-button>
            </ic-menu-group>
            <ic-menu-button
              label="Actions"
              submenu-trigger-for="abc"
            ></ic-menu-button>
          </ic-popover-menu>
          <ic-popover-menu submenu-id="abc" menu-aria-label="submenu popover">
            <ic-menu-button label="Edit"></ic-menu-button>
            <ic-menu-button
              label="Find"
              submenu-trigger-for="abc123"
            ></ic-menu-button>
            <ic-menu-button
              label="Delete"
              variant="destructive"
            ></ic-menu-button>
          </ic-popover-menu>
          <ic-popover-menu
            submenu-id="abc123"
            menu-aria-label="3rd level submenu popover"
          >
            <ic-menu-button
              label="Search the web"
              description="This will search the web to find the thing you are looking for."
            ></ic-menu-button>
            <ic-menu-button label="Find..."></ic-menu-button>
            <ic-menu-button label="Find icons">
              <svg
                slot="icon"
                xmlns="http://www.w3.org/2000/svg"
                height="24px"
                viewBox="0 0 24 24"
                width="24px"
              >
                <path d="M0 0h24v24H0V0z" fill="none" />
                <path
                  d="M17.65 6.35C16.2 4.9 14.21 4 12 4c-4.42 0-7.99 3.58-7.99 8s3.57 8 7.99 8c3.73 0 6.84-2.55 7.73-6h-2.08c-.82 2.33-3.04 4-5.65 4-3.31 0-6-2.69-6-6s2.69-6 6-6c1.66 0 3.14.69 4.22 1.78L13 11h7V4l-2.35 2.35z"
                />
              </svg>
            </ic-menu-button>
            <ic-menu-button
              label="Show found results"
              variant="toggle"
            ></ic-menu-button>
          </ic-popover-menu>
        </div>
      `
    }
  </Story>
</Canvas>

### Left placement

<Canvas>
  <Story name="Left placement" parameters={{ loki: { skip: true } }}>
    {(args) =>
      html`
        <div style="margin-left: 1000px">
          <ic-button id="button-1" onclick="buttonClick()"
            >Show popover</ic-button
          >
          <script>
            var icPopover = document.querySelector("ic-popover-menu");
            function buttonClick() {
              icPopover.open = true;
            }
          </script>
          <div>
            <ic-popover-menu anchor="button-1" menu-aria-label="popover">
              <ic-menu-button
                label="Copy code"
                disabled="true"
              ></ic-menu-button>
              <ic-menu-group label="View">
                <ic-menu-button
                  label="Zoom in"
                  keyboard-shortcut="Cmd+"
                ></ic-menu-button>
                <ic-menu-button
                  label="Zoom out"
                  keyboard-shortcut="Cmd-"
                ></ic-menu-button>
              </ic-menu-group>
              <ic-menu-button
                label="Actions"
                submenu-trigger-for="abc"
              ></ic-menu-button>
            </ic-popover-menu>
            <ic-popover-menu submenu-id="abc" menu-aria-label="submenu popover">
              <ic-menu-button label="Edit"></ic-menu-button>
              <ic-menu-button
                label="Find"
                submenu-trigger-for="abc123"
              ></ic-menu-button>
              <ic-menu-button
                label="Delete"
                variant="destructive"
              ></ic-menu-button>
            </ic-popover-menu>
            <ic-popover-menu
              submenu-id="abc123"
              menu-aria-label="3rd level submenu popover"
            >
              <ic-menu-button
                label="Search the web"
                description="This will search the web to find the thing you are looking for."
              ></ic-menu-button>
              <ic-menu-button label="Find..."></ic-menu-button>
              <ic-menu-button label="Find icons">
                <svg
                  slot="icon"
                  xmlns="http://www.w3.org/2000/svg"
                  height="24px"
                  viewBox="0 0 24 24"
                  width="24px"
                >
                  <path d="M0 0h24v24H0V0z" fill="none" />
                  <path
                    d="M17.65 6.35C16.2 4.9 14.21 4 12 4c-4.42 0-7.99 3.58-7.99 8s3.57 8 7.99 8c3.73 0 6.84-2.55 7.73-6h-2.08c-.82 2.33-3.04 4-5.65 4-3.31 0-6-2.69-6-6s2.69-6 6-6c1.66 0 3.14.69 4.22 1.78L13 11h7V4l-2.35 2.35z"
                  />
                </svg>
              </ic-menu-button>
              <ic-menu-button
                label="Show found results"
                variant="toggle"
              ></ic-menu-button>
            </ic-popover-menu>
          </div>
        </div>
      `
    }
  </Story>
</Canvas>

### Top placement

<Canvas>
  <Story name="Top placement" parameters={{ loki: { skip: true } }}>
    {(args) =>
      html`
        <div style="margin-top: 540px">
          <ic-button id="button-1" onclick="buttonClick()"
            >Show popover</ic-button
          >
          <script>
            var icPopover = document.querySelector("ic-popover-menu");
            function buttonClick() {
              icPopover.open = true;
            }
          </script>
          <div>
            <ic-popover-menu anchor="button-1" menu-aria-label="popover">
              <ic-menu-button
                label="Copy code"
                disabled="true"
              ></ic-menu-button>
              <ic-menu-group label="View">
                <ic-menu-button
                  label="Zoom in"
                  keyboard-shortcut="Cmd+"
                ></ic-menu-button>
                <ic-menu-button
                  label="Zoom out"
                  keyboard-shortcut="Cmd-"
                ></ic-menu-button>
              </ic-menu-group>
              <ic-menu-button
                label="Actions"
                submenu-trigger-for="abc"
              ></ic-menu-button>
            </ic-popover-menu>
            <ic-popover-menu submenu-id="abc" menu-aria-label="submenu popover">
              <ic-menu-button label="Edit"></ic-menu-button>
              <ic-menu-button
                label="Find"
                submenu-trigger-for="abc123"
              ></ic-menu-button>
              <ic-menu-button
                label="Delete"
                variant="destructive"
              ></ic-menu-button>
            </ic-popover-menu>
            <ic-popover-menu
              submenu-id="abc123"
              menu-aria-label="3rd level submenu popover"
            >
              <ic-menu-button
                label="Search the web"
                description="This will search the web to find the thing you are looking for."
              ></ic-menu-button>
              <ic-menu-button label="Find..."></ic-menu-button>
              <ic-menu-button label="Find icons">
                <svg
                  slot="icon"
                  xmlns="http://www.w3.org/2000/svg"
                  height="24px"
                  viewBox="0 0 24 24"
                  width="24px"
                >
                  <path d="M0 0h24v24H0V0z" fill="none" />
                  <path
                    d="M17.65 6.35C16.2 4.9 14.21 4 12 4c-4.42 0-7.99 3.58-7.99 8s3.57 8 7.99 8c3.73 0 6.84-2.55 7.73-6h-2.08c-.82 2.33-3.04 4-5.65 4-3.31 0-6-2.69-6-6s2.69-6 6-6c1.66 0 3.14.69 4.22 1.78L13 11h7V4l-2.35 2.35z"
                  />
                </svg>
              </ic-menu-button>
              <ic-menu-button
                label="Show found results"
                variant="toggle"
              ></ic-menu-button>
            </ic-popover-menu>
          </div>
        </div>
      `
    }
  </Story>
</Canvas>
